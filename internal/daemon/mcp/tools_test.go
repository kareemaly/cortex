...
func setupArchitectWithDaemon(t *testing.T, windowExists bool) (*Server, *ticket.Store, func()) {
	t.Helper()
...
func TestHandleListTickets(t *testing.T) {
	server, store, cleanup := setupArchitectWithDaemon(t, true)
	defer cleanup()

	// Create some tickets
	_, _ = store.Create("Ticket 1", "body 1", "", nil)
	_, _ = store.Create("Ticket 2", "body 2", "", nil)
...
func TestHandleListTicketsWithStatus(t *testing.T) {
	...
	t1, _ := store.Create("Backlog Ticket", "", "", nil)
	t2, _ := store.Create("Progress Ticket", "", "", nil)
...
func TestHandleListTicketsWithQuery(t *testing.T) {
	...
	// Create tickets
	_, _ = store.Create("Fix login bug", "Authentication issue", "", nil)
	_, _ = store.Create("Add feature", "New feature", "", nil)
...
func TestHandleListTicketsWithStatusAndQuery(t *testing.T) {
	...
	// Create tickets
	t1, _ := store.Create("Fix login bug", "Authentication issue", "", nil)
	t2, _ := store.Create("Fix login feature", "Another login issue", "", nil)
	_ = store.Move(t2.ID, ticket.StatusProgress)
...
func TestHandleListTickets_MissingStatus(t *testing.T) {
	server, _, cleanup := setupArchitectWithDaemon(t, true)
	defer cleanup()
...
func TestHandleReadTicket(t *testing.T) {
	...
	created, _ := store.Create("Test Ticket", "body", "", nil)
...
func TestHandleUpdateTicket(t *testing.T) {
	...
	created, _ := store.Create("Original", "body", "", nil)
	newTitle := "Updated"
...
func TestHandleDeleteTicket(t *testing.T) {
	...
	created, _ := store.Create("To Delete", "", "", nil)
...
func TestHandleMoveTicket(t *testing.T) {
	...
	created, _ := store.Create("Test", "", "", nil)
...
func TestHandleSpawnSession(t *testing.T) {
	...
	created, err := store.Create("Test Spawn Session", "Test body", "", nil)
	if err != nil {
...
func TestHandleSpawnSessionActiveSession(t *testing.T) {
	...
	created, err := store.Create("Test Active Session", "Test body", "", nil)
	if err != nil {
		t.Fatalf("failed to create ticket: %v", err)
	}
	_, err = store.SetSession(created.ID, "claude", "window", nil, nil)
	if err != nil {
...
func TestHandleSpawnSessionAutoMovesToProgress(t *testing.T) {
	...
	created, err := store.Create("Test Auto Move", "Test body", "", nil)
	if err != nil {
		t.Fatalf("failed to create ticket: %v", err)
	}
...
func setupTicketSession(t *testing.T) (*Server, string, func()) {
	...
	// Create a ticket
	tk, err := store.Create("Test Ticket", "body", "", nil)
	if err != nil {
		_ = os.RemoveAll(tmpDir)
...
func TestHandleSpawnSession_StateActive_AllModes(t *testing.T) {
	...
	for _, mode := range modes {
		t.Run("mode="+mode, func(t *testing.T) {
			...
			created, _ := store.Create("Test Ticket", "body", "", nil)
			_, _ = store.SetSession(created.ID, "claude", "window", nil, nil)
...
func TestHandleSpawnSession_StateOrphaned_ModeNormal(t *testing.T) {
	server, store, cleanup := setupArchitectWithDaemon(t, false)
	defer cleanup()
