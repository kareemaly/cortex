...
func (h *MetaHandlers) Spawn(w http.ResponseWriter, r *http.Request) {
	...
	case spawn.StateOrphaned:
		// Auto-clean orphaned session, then spawn fresh
		if h.deps.MetaSessionManager != nil {
			store := h.deps.MetaSessionManager.GetStore()
	...
	h.spawnMetaSession(w, r)
}

// spawnMetaSession spawns a fresh meta session.
func (h *MetaHandlers) spawnMetaSession(w http.ResponseWriter, r *http.Request) {
